...
XuerichSteelSensitiveDetector::XuerichSteelSensitiveDetector(G4String hName): G4VSensitiveDetector(hName)
{
	collectionName.insert("SteelHitsCollection");
}

XuerichSteelSensitiveDetector::~XuerichSteelSensitiveDetector()
{
}

void XuerichSteelSensitiveDetector::Initialize(G4HCofThisEvent* pHitsCollectionOfThisEvent)
{
	m_pSteelHitsCollection = new XuerichSteelHitsCollection(SensitiveDetectorName, collectionName[0]);
	...
	pHitsCollectionOfThisEvent->AddHitsCollection(iHitsCollectionID, m_pSteelHitsCollection);

	m_hParticleTypes.clear();
}

G4bool XuerichSteelSensitiveDetector::ProcessHits(G4Step* pStep, G4TouchableHistory *pHistory)
{
	G4double dEnergyDeposited = pStep->GetTotalEnergyDeposit();
	G4Track *pTrack = pStep->GetTrack();

	XuerichSteelHit* pHit = new XuerichSteelHit();
	...
	m_pSteelHitsCollection->insert(pHit);

	return true;
}

void XuerichSteelSensitiveDetector::EndOfEvent(G4HCofThisEvent *pHitsCollectionOfThisEvent)
{
//  if (verboseLevel>0) { 
