...
XuerichAlSensitiveDetector::XuerichAlSensitiveDetector(G4String hName): G4VSensitiveDetector(hName)
{
	collectionName.insert("AlHitsCollection");
}

XuerichAlSensitiveDetector::~XuerichAlSensitiveDetector()
{
}

void XuerichAlSensitiveDetector::Initialize(G4HCofThisEvent* pHitsCollectionOfThisEvent)
{
	m_pAlHitsCollection = new XuerichAlHitsCollection(SensitiveDetectorName, collectionName[0]);
	...
	pHitsCollectionOfThisEvent->AddHitsCollection(iHitsCollectionID, m_pAlHitsCollection);

	m_hParticleTypes.clear();
}

G4bool XuerichAlSensitiveDetector::ProcessHits(G4Step* pStep, G4TouchableHistory *pHistory)
{
	G4double dEnergyDeposited = pStep->GetTotalEnergyDeposit();
	G4Track *pTrack = pStep->GetTrack();

	XuerichAlHit* pHit = new XuerichAlHit();
	...
	m_pAlHitsCollection->insert(pHit);

	return true;
}

void XuerichAlSensitiveDetector::EndOfEvent(G4HCofThisEvent *pHitsCollectionOfThisEvent)
{
//  if (verboseLevel>0) { 
