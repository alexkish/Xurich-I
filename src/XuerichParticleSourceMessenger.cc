...
#include "XuerichParticleSourceMessenger.hh"

XuerichParticleSourceMessenger::XuerichParticleSourceMessenger(XuerichParticleSource *pParticleSource):
	m_pParticleSource(pParticleSource), m_bShootIon(false)
{
	...
	m_pAngTypeCmd->SetCandidates("None iso direction");
	...
	m_pEnergyTypeCmd->SetCandidates("None Mono Kr83m");

	// external file with energy, position and momentum
	m_pExtFileCmd = new G4UIcmdWithAString("/xe/gun/extfile", this);
	m_pExtFileCmd->SetGuidance("File containing energy, position and momentum");
	m_pExtFileCmd->SetParameterName("ExternalFile", false);
...
XuerichParticleSourceMessenger::~XuerichParticleSourceMessenger()
{
	delete m_pTypeCmd;
	delete m_pShapeCmd;
	delete m_pCenterCmd;
	...
	delete m_pEnergyCmd;
	delete m_pListCmd;
	delete m_pExtFileCmd;

	delete m_pDirectory;
}

void
XuerichParticleSourceMessenger::SetNewValue(G4UIcommand * command, G4String newValues)
{
	...
	else if(command == m_pExtFileCmd)
		m_pParticleSource->SetExtFile(newValues);
