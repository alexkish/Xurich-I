...
XuerichNaISensitiveDetector::XuerichNaISensitiveDetector(G4String hName): G4VSensitiveDetector(hName)
{
	collectionName.insert("NaIHitsCollection");
}

XuerichNaISensitiveDetector::~XuerichNaISensitiveDetector()
{
}

void XuerichNaISensitiveDetector::Initialize(G4HCofThisEvent* pHitsCollectionOfThisEvent)
{
	m_pNaIHitsCollection = new XuerichNaIHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool XuerichNaISensitiveDetector::ProcessHits(G4Step* pStep, G4TouchableHistory *pHistory)
{
	G4double dEnergyDeposited = pStep->GetTotalEnergyDeposit();
	G4Track *pTrack = pStep->GetTrack();

	XuerichNaIHit* pHit = new XuerichNaIHit();
...
void XuerichNaISensitiveDetector::EndOfEvent(G4HCofThisEvent *pHitsCollectionOfThisEvent)
{
//  if (verboseLevel>0) { 
