#ifndef __XENON10PLEADHIT_H__
#define __XENON10PLEADHIT_H__
...
class XuerichLeadHit: public G4VHit
{
public:
	XuerichLeadHit();
	~XuerichLeadHit();
	XuerichLeadHit(const XuerichLeadHit &);
	const XuerichLeadHit & operator=(const XuerichLeadHit &);
	G4int operator==(const XuerichLeadHit &) const;
...
typedef G4THitsCollection<XuerichLeadHit> XuerichLeadHitsCollection;

extern G4Allocator<XuerichLeadHit> XuerichLeadHitAllocator;

inline void*
XuerichLeadHit::operator new(size_t)
{
	return((void *) XuerichLeadHitAllocator.MallocSingle());
}

inline void
XuerichLeadHit::operator delete(void *pXuerichLeadHit)
{
	XuerichLeadHitAllocator.FreeSingle((XuerichLeadHit*) pXuerichLeadHit);
}
