#ifndef __XENON10PSTEELHIT_H__
#define __XENON10PSTEELHIT_H__
...
class XuerichSteelHit: public G4VHit
{
public:
	XuerichSteelHit();
	~XuerichSteelHit();
	XuerichSteelHit(const XuerichSteelHit &);
	const XuerichSteelHit & operator=(const XuerichSteelHit &);
	G4int operator==(const XuerichSteelHit &) const;
...
typedef G4THitsCollection<XuerichSteelHit> XuerichSteelHitsCollection;

extern G4Allocator<XuerichSteelHit> XuerichSteelHitAllocator;

inline void*
XuerichSteelHit::operator new(size_t)
{
	return((void *) XuerichSteelHitAllocator.MallocSingle());
}

inline void
XuerichSteelHit::operator delete(void *pXuerichSteelHit)
{
	XuerichSteelHitAllocator.FreeSingle((XuerichSteelHit*) pXuerichSteelHit);
}
