#ifndef __XENON10PALHIT_H__
#define __XENON10PALHIT_H__
...
class XuerichAlHit: public G4VHit
{
public:
	XuerichAlHit();
	~XuerichAlHit();
	XuerichAlHit(const XuerichAlHit &);
	const XuerichAlHit & operator=(const XuerichAlHit &);
	G4int operator==(const XuerichAlHit &) const;
...
typedef G4THitsCollection<XuerichAlHit> XuerichAlHitsCollection;

extern G4Allocator<XuerichAlHit> XuerichAlHitAllocator;

inline void*
XuerichAlHit::operator new(size_t)
{
	return((void *) XuerichAlHitAllocator.MallocSingle());
}

inline void
XuerichAlHit::operator delete(void *pXuerichAlHit)
{
	XuerichAlHitAllocator.FreeSingle((XuerichAlHit*) pXuerichAlHit);
}
